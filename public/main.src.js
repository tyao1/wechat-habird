...
var TEXT_TOTAL_TIME_ELAPSED = '本次累计被续 %s 秒';
...
var notisVoice = document.getElementById('notisVoice');
var notVTimeout;
function clearV() {
  notisVoice.innerHTML = '';
}
function buildNotiV(user) {
  clearTimeout(notVTimeout);
  var str = '<div class="noti">'+
    '<img src="' + user.headimgurl + '" class="avatar">'+
    '<span class="notiName">' + user.nickname + '</span>'+
    '<span>的语音膜！</span>'+
  '</div>';
  notisVoice.innerHTML = str;
  notVTimeout = setTimeout(clearV, 3500);
}
...
var notIOS = !u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/);
function runNow() {
...
function getBufs(src) {
  ...
  xhr1.addEventListener('load', runNow);
  xhr1.send();
}

function playRandomVoice () {
  superagent.get('./randomVoice')
    .end(function(err, res) {
      if (err) return;
      var src = res.body.src;
      if (!src) return;
      function playAudio() {
        if (notIOS && context) getBufs(src);
        else {
      ...
      if (window.WeixinJSBridge) {
        WeixinJSBridge.invoke('getNetworkType', {}, playAudio, false);
      } else{
        document.addEventListener("WeixinJSBridgeReady", function() {
            WeixinJSBridge.invoke('getNetworkType', {}, playAudio);
        }, false);
      }
      buildNotiV(res.body.user);
    });
}
...
function setGameOver() {
  ...
  playHurtSound();
  playRandomVoice();
  socket.emit('gameover', {plus: _score, minus: _timeElapsed});
}
...
function showGameOver() {
  ...
  _shareScoreText.renderable = true;

  if (!wx) return alert('今天被微信分享得罪了一下');
  var shareWithDesc = {
    title: '轮到你给长者续命了！能击败我吗？' , // 分享标题
    desc: '穿上红色的衣服！我奉献了' + _timeElapsed + '秒生命，为长者续了' + _score + '秒！', // 分享描述
  ...
  wx.onMenuShareTimeline({
    title: '穿上红色的衣服！我奉献了' + _timeElapsed + '秒生命，为长者续了' + _score + '秒！' , // 分享标题
    link: 'http://moha.taskbee.cn/', // 分享链接
    imgUrl: 'http://moha.taskbee.cn/images/frog.png', // 分享图标
    success: function () {
      playScoreSound();
  ...
  wx.onMenuShareAppMessage(shareWithDesc);
  wx.onMenuShareQQ(shareWithDesc);
  wx.onMenuShareWeibo(shareWithDesc);
  wx.onMenuShareQZone(shareWithDesc);
}
...
function hideGameOver() {
  ...
  _shareScoreText.renderable = false;
...
function initTexts() {
  ...
  _shareScoreText = _game.add.text(
    _game.world.width,
    32,
    '转发或分享，和朋友谈笑风生⬆️',
    {
  ...
  _shareScoreText.anchor.setTo(1, 1);
  _shareScoreSprite = createTextSprite(_shareScoreText);
  _shareScoreSprite.inputEnabled = true;
...
document.addEventListener("WeixinJSBridgeReady", function() {
  ...
  initGame({
    ...
    feedbackFunc: function() {
        window.location = 'https://taskbee.cn/feedback';
    },
